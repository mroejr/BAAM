...
Users will be walked throught the [Kodi Add-on Manager](https://kodi.wiki/view/Add-on_manager). In this wiki page it has a step by step process that is shown. The user has the ability to set the updates to be manual or automatic. If automatic is chosen then Kodi constantly is polling the Add-on repositories. Kodi fetches the checksum and compares that to the previously fetched. If there is a difference Kodi will fetch the /datadir/addon.id/files and Kodi is looking for the file to be in a zip format. It doesn't show that the zip format is a requirement. The Add-on requirements are described under [Requirements for Add-ons](https://kodi.wiki/view/Add-on_repositories). Under these requirements Kodi shows that Add-on repositories only need to be HTTP and then Kodi asks if the repository will run SSL. This is not a requirement for the Add-ons to use, the users will be alerted that no security measures have been taken. 
